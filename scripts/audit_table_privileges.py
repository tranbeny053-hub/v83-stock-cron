...
- the first statement makes the transaction READ ONLY; the audit refuses unless the server says so;
...
POLICY_COMMANDS = {"SELECT": "SELECT", "INSERT": "INSERT", "UPDATE": "UPDATE", "DELETE": "DELETE"}
...
# The owner of a relation or schema is a role name. Only standard identifiers are reported.
ROLE_NAME_COLUMNS = {
...
REHEARSAL_EXPECTED_ANON = {
    ...
    "public.analysis_timeframe_results": dict.fromkeys(API_PRIVILEGES, "NO_PRIVILEGE"),
    ...
    "public.provider_observations": dict.fromkeys(API_PRIVILEGES, "OPEN"),
...
REHEARSAL_EXPECTED_EXPOSURE_MARKERS = (
    ...
    "public.analysis_timeframe_results.run_id: anon SELECT COLUMN GRANT OPEN",
...
def rehearsal_failures(observed: Mapping[str, Any], assessment: Mapping[str, Any]) -> list[str]:
    ...
    for key, expected in REHEARSAL_EXPECTED_ANON.items():
        got = assessment["tables"].get(key, {}).get("roles", {}).get("anon", {}).get("data_api")
        if got != expected:
            failures.append(f"{key} anon is {got}, not {expected}")
    service = assessment["tables"].get("public.predictions", {}).get("roles", {}).get(
        "service_role", {}
    )
    ...
    watchlist_service = assessment["tables"].get("public.watchlist", {}).get("roles", {}).get(
        "service_role", {}
    )
    ...
    if any("public.predictions:" in line for line in exposures):
        failures.append("RLS without a policy must not count as an exposure")
...
def _redact_role_names(name: str, rows: list[list[Any]]) -> list[list[Any]]:
    for row in rows:
        for index in ROLE_NAME_COLUMNS.get(name, ()):
            value = row[index]
            if not (isinstance(value, str) and _IDENTIFIER.match(value)):
                row[index] = "<non-standard role name withheld>"
    return rows


def _policy_roles(text: Any) -> set[str]:
    """``pg_policies.roles`` as text, e.g. ``{public}`` or ``{anon,authenticated}``."""
...
def assess(observed: Mapping[str, Any]) -> dict[str, Any]:
    ...
    A table is EXPOSED to a role for a command when the role holds that privilege AND nothing
    filters rows: row-level security is off, or the role bypasses it, or a policy for the role and
    command exists (then the policy expression decides, and it is reported for review). The Data API
    ...
    roles = {
        row[0]: {"bypassrls": bool(row[2]), "superuser": bool(row[1])}
        for row in observed["roles"]
    }
    usage = {(row[0], row[2]): bool(row[3]) for row in observed["schema_usage"]}
    effective: dict[tuple[str, str, str], set[str]] = {}
    for schema, table, role, privilege, held in observed["effective_privileges"]:
        if held:
            effective.setdefault((schema, table, role), set()).add(privilege)
    ...
    for schema, table, relkind, owner, rls, forced, owned_by_current in observed["tables"]:
        ...
        entry: dict[str, Any] = {
            ...
            "policies": [row[2] for row in policies.get((schema, table), [])],
            "realtime_publications": [
                row[0]
        ...
        for role in API_ROLES:
            held = effective.get((schema, table, role), set())
            role_info = roles.get(role, {})
            bypass = bool(role_info.get("bypassrls")) or bool(role_info.get("superuser"))
            reachable = schema in served and usage.get((schema, role), False)
            commands: dict[str, str] = {}
            for command in API_PRIVILEGES:
                if command not in held:
                    commands[command] = "NO_PRIVILEGE"
                elif not rls or bypass:
                    commands[command] = "OPEN" if reachable else "OPEN_BUT_NOT_API_REACHABLE"
                else:
                    # Only a PERMISSIVE policy can admit rows; RESTRICTIVE ones only narrow them.
                    applicable = [
                        row[2]
                        for row in policies.get((schema, table), [])
                        if (role in _policy_roles(row[4]) or "public" in _policy_roles(row[4]))
                        and row[5] in (POLICY_COMMANDS[command], "ALL")
                        and str(row[3]).upper() == "PERMISSIVE"
                    ]
                    if not applicable:
                        commands[command] = "RLS_DENIES_ALL"
                    elif reachable:
                        commands[command] = "POLICY_DECIDES: " + ", ".join(sorted(applicable))
                    else:
                        commands[command] = "POLICY_DECIDES_NOT_API_REACHABLE"
            entry["roles"][role] = {
                "privileges": sorted(held),
                "bypasses_row_level_security": bypass,
                "schema_usage": usage.get((schema, role), False),
                "data_api": commands,
            }
            if role != "service_role":
                for command, verdict in commands.items():
                    if verdict == "OPEN" or verdict.startswith("POLICY_DECIDES: "):
                        exposures.append(f"{key}: {role} {command} {verdict}")
                if commands["SELECT"] == "OPEN" and entry["realtime_publications"]:
                    exposures.append(
        ...
        tables[key] = entry
    table_facts = {
        (row[0], row[1]): {"rls": bool(row[4])} for row in observed["tables"]
    }
    for schema, table, column, grantee, privilege in observed["column_grants"]:
        if grantee not in ("anon", "authenticated") or privilege not in API_PRIVILEGES:
            continue
        role_info = roles.get(grantee, {})
        bypass = bool(role_info.get("bypassrls")) or bool(role_info.get("superuser"))
        reachable = schema in served and usage.get((schema, grantee), False)
        rls = table_facts.get((schema, table), {}).get("rls", False)
        if reachable and (not rls or bypass):
            exposures.append(f"{schema}.{table}.{column}: {grantee} {privilege} COLUMN GRANT OPEN")
        elif reachable:
            exposures.append(
                f"{schema}.{table}.{column}: {grantee} {privilege} COLUMN GRANT UNDER RLS, REVIEW"
            )
    for view_schema, view, role, privilege, held in observed.get("dependent_view_privileges", []):
        if held and role != "service_role" and view_schema in served:
            exposures.append(
    ...
    return {
        ...
        "verdict": "EXPOSED" if exposures else "NOT_EXPOSED_THROUGH_TABLE_GRANTS",
        "not_audited": [
