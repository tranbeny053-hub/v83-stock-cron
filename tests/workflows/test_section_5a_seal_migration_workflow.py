"""The seal-migration workflow, read and EXECUTED the way GitHub runs it (owner ruling K2=A).

It applies migration 0009 once, with the database secret, so it carries every guard the evaluation
workflow earned: no expression in shell source (V808-F1), `shell: bash` everywhere (V808-R6), the
exact runtime and the closed trust boundary (E3=A, G1=A, J1=B). Where the two workflows must be
identical, this file compares them to each other rather than to a copy of the text.
"""
...
PATH = ROOT / provenance.SEAL_MIGRATION_WORKFLOW
...
ENTRYPOINT = ("-I", "-S", "-B", seal.SCRIPT)
RUNNER_TEMP = "/home/runner/work/_temp"
WHEELHOUSE = f"{RUNNER_TEMP}/section-5a-wheels"
REPORT = "section-5a-seal-migration-report.json"
...
APPLY = _step(STEPS, "Apply migration 0009")
...
def test_it_is_the_workflow_provenance_binds_and_it_is_dispatch_only() -> None:
    assert PATH.is_file()
...
def test_both_inputs_are_required_strings_and_the_token_is_the_script_s() -> None:
    ...
    assert seal.CONFIRMATION in confirm


def test_it_can_never_run_beside_an_evaluation_run() -> None:
...
def test_every_python_process_is_isolated_or_is_the_bytecode_free_test_run() -> None:
    for step in (ATTEST, APPLY):
        assert step.run.startswith(f"python -I -S -B {seal.SCRIPT} "), step.name
    invocations = 0
    for step in RUN_STEPS:
...
def test_the_in_job_tests_cover_the_script_the_migration_and_this_boundary() -> None:
    ...
    assert paths == [
        "tests/scripts/test_apply_section_5a_seal.py",
        "tests/migrations/test_section_5a_seal_migration.py",
        "tests/workflows/test_section_5a_seal_migration_workflow.py",
        "tests/workflows/test_workflow_steps_reader.py",
        "tests/oos/evaluation/test_provenance.py",
    ]
    for path in paths:
...
def test_the_real_parser_binds_each_hostile_value_to_its_own_option(
    ...
    args = seal.build_parser().parse_args(call["argv"][len(ENTRYPOINT) :])
    assert args.mode == seal.MODE_APPLY, "the mode is fixed in the workflow, never an input"
